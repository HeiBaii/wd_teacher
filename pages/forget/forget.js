...
Page({
  ...
  tapCode: common.throttle(function(e) {
    ...
    if (is_phone === true) {
      ...
      let that = this;
      http.postReq(app.globalData.config.REQUEST_URL+'/user/mobile/verify', { "mobile": that.data.input.telphone, "type": 1},function(res){
        if (res.code != 0) {
          app.toast(res.msg);
          return
        }else{
  ...
  setPwdBtn: common.throttle(function() {
    let that = this;
    //手机验证码校验
    let data = { "mobile": that.data.input.telphone,"type": 1,"verifyCode": that.data.input.code};
    http.postReq(app.globalData.config.REQUEST_URL+'/user/validate/verify',data,function(res){
      if (res.code === 0) {
        //设置密码
        let data = {
          "hashNum": res.data,
          "mobile": that.data.input.telphone,
          "password": that.data.input.password,
          "rePassword": that.data.input.repassword
        };
        http.postReq(app.globalData.config.REQUEST_URL+'/user/update/password',data,function(res){
          if (res.code === 0) {
            app.toast("设置密码成功")
            setTimeout(function(){
              app.goBack()
